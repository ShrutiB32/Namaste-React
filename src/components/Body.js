      ...
      <div className="resto-container">
        {
          ...
          //ep06 part06 here we change the listofrestaurants to filteredRestaurants becuase listofrestaurants contains copy of all the restaurants and filteredRestaurants contains only filtered list but we dont get anything on screen becuase wen we defined the useState we gave default value of filteredRestaurants as [] and so we need to update wen we fetch the data
          filteredRestaurant.map((restaurant) => {
            console.log(restaurant);
            return (
              // <RestaurantCard key={restaurant?.data.id} resData={restaurant} /> ep 07 part 05
              <Link to={"/restaurants/" + restaurant?.info?.id} key={restaurant?.info?.id}>
                <RestaurantCard  resData={restaurant} />
              </Link>
              //since we are mapping the restaurant card over the link the key should attached to parent jsx i.e  link ep 07 part 05
            );
          })
        }
      </div>
